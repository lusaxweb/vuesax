<template lang="html">
  <li v-show="visible" :data-text="text" class="vs-component" v-on="listeners">
    <button
      ...
      :style="styles"
      :class="{
        activex: $parent.parent.multiple
    ...
    >
      <vs-icon
        ...
        icon="check_circle"
      ></vs-icon>
      <span v-if="!useSlot" v-html="getText"></span>
      <slot />
    </button>
  </li>
</template>
    ...
    disabledx() {
      if (this.$parent.parent.multiple) {
        if (this.isActive) {
          ...
          return (
            this.$parent.parent.maxSelected == this.$parent.parent.value.length
          );
        }
      } else {
        return false;
    ...
    styles() {
      return {
        background: this.isActive
          ? _color.getColor(this.$parent.parent.color, 0.1)
          : null,
        color: this.isActive
          ? _color.getColor(this.$parent.parent.color, 1)
          : null,
  ...
  methods: {
    ...
    backspace() {
      if (this.$parent.parent.autocomplete) {
        ...
        this.$parent.parent.$refs.inputselect.value = valueInput.substr(
          0,
          valueInput.length - 1
        );
