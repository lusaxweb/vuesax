  ...
  export default {
      ...
      listeners() {
        return {
          ...
          blur: (event) => {
            ...
            if (
              (this.multiple || this.autocomplete) &&
              (!event.relatedTarget || !event.relatedTarget.classList.contains('vs-select--item'))
            ) {
              this.closeOptions()
            } else {
    ...
    methods: {
      ...
      focus() {
        ...
        if (!this.autocomplete && this.multiple) {
          setTimeout(() => {
            const el = this.$el.querySelector('.vs-select--input')
            if (el) el.focus()
          }, 50);
        }
